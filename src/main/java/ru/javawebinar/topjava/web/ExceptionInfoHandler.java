...
public class ExceptionInfoHandler {
    ...
    public ErrorInfo bindValidationError(HttpServletRequest request, BindException e) {
        String[] details = e.getBindingResult().getFieldErrors().stream().map(fe ->
            String.format("[%s] %s", fe.getField(), fe.getDefaultMessage())).toArray(String[]::new);
    ...
    private static ErrorInfo logAndGetErrorInfo(HttpServletRequest req, Exception e, boolean logException,
        ...
        return new ErrorInfo(req.getRequestURL(), errorType, details.length == 0 ?
            new String[] {rootCause.toString()} : details);
    }
}